package com.sulake.core.window.services
{
   ...
   public class WindowMouseOperator implements IDisposable
   {
      ...
      protected var var_145:DisplayObject;
      ...
      public function begin(param1:IWindow, param2:uint = 0) : IWindow
      {
         ...
         {
            this.var_145.addEventListener(MouseEvent.MOUSE_DOWN,this.handler,false);
            this.var_145.addEventListener(MouseEvent.MOUSE_UP,this.handler,false);
            this.var_145.addEventListener(Event.ENTER_FRAME,this.handler);
            this._mouse.x = this.var_145.mouseX;
            this._mouse.y = this.var_145.mouseY;
      ...
      public function end(param1:IWindow) : IWindow
      {
         ...
         if(this._working)
         {
            ...
            {
               this.var_145.removeEventListener(MouseEvent.MOUSE_DOWN,this.handler,false);
               this.var_145.removeEventListener(MouseEvent.MOUSE_UP,this.handler,false);
               this.var_145.removeEventListener(Event.ENTER_FRAME,this.handler);
      ...
      protected function handler(param1:Event) : void
      {
         ...
         if(this._working)
         {
            if(param1.type == Event.ENTER_FRAME)
            {
               ...
               else if(this._mouse.x != this.var_145.mouseX || this._mouse.y != this.var_145.mouseY)
               {
                  this.operate(this.var_145.mouseX,this.var_145.mouseY);
                  this._mouse.x = this.var_145.mouseX;
                  this._mouse.y = this.var_145.mouseY;
               }
               return;
      ...
      public function operate(param1:int, param2:int) : void
      {
         ...
         this.getMousePositionRelativeTo(this._window,this._mouse,this.var_696);
         this._window.offset(this.var_696.x - this._offset.x,this.var_696.y - this._offset.y);
      }
