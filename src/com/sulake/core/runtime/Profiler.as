package com.sulake.core.runtime
{
   ...
   class Profiler extends Component implements IProfiler
   {
      ...
      {
         this.var_298 = new Dictionary(true);
         super(param1,0,null);
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            {
               IDisposable(this.var_298[_loc1_]).dispose();
               delete this.var_298[_loc1_];
            }
            super.dispose();
         }
      }
      ...
      {
         this.var_2901 = getTimer() - this.var_2048;
      ...
      public function getProfilerAgentsInArray() : Array
      {
         ...
         for(_loc3_ in this.var_298)
         {
            _loc2_ = _loc3_ as IUpdateReceiver;
            if(IDisposable(_loc3_).disposed)
            {
               IDisposable(this.var_298[_loc3_]).dispose();
               delete this.var_298[_loc3_];
            }
            else
            {
               _loc1_.push(this.var_298[_loc3_]);
            }
         }
         return _loc1_;
      }
      ...
      {
         var _loc2_:ProfilerAgent = this.var_298[param1];
