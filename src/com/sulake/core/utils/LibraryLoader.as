package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1721:uint = 1;
      
      protected static const const_1719:uint = 2;
      
      protected static const const_1722:uint = 4;
      
      protected static const const_1720:uint = 3;
      ...
      public static const const_1147:Boolean = false;
      ...
      protected var var_2880:String = "";
      
      protected var var_2879:String = "";
      
      protected var var_2111:int;
      
      protected var var_1320:int;
      
      protected var var_1319:int;
      
      protected var var_2953:uint = 0;
      
      protected var var_2881:Boolean = true;
      ...
      protected static function throttle() : void
      {
         ...
         if(const_1147)
         {
            ...
            while(_loc2_ > -1)
            {
               _loc1_ = var_816[_loc2_] as LibraryLoader;
               if(_loc1_ && (_loc1_.ready || _loc1_.disposed))
               {
                  var_816.splice(_loc2_,1);
               }
               _loc2_--;
            }
            while(var_816.length < const_1595 && false)
            {
               _loc1_ = var_1006.shift() as LibraryLoader;
      ...
      {
         return !!this.var_1067 ? uint(this.var_1320 - this.var_2111) : uint(getTimer() - this.var_2111);
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,this.var_407,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
            try
            {
               this.var_42.close();
            }
      ...
      {
         if(!this.var_1067 && !_disposed && this.var_1319 > 0)
         {
            try
            {
               this.var_42.close();
            ...
            this.addRequestCounterToUrlRequest(this.var_242,const_1481 - this.var_1319);
            if(this.var_2881)
            {
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               this.var_407 = HTTPStatusEvent(param1).status;
               this.debug("Load event STATUS " + this.var_407 + " for file \"" + this.url + "\"");
               break;
            case Event.UNLOAD:
               this.debug("Load event UNLOAD for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,this.var_407,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               this.debug("Load event PROGRESS for file \"" + this.url + "\"  bytes: " + this.bytesLoaded + "/" + this.bytesTotal);
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,this.var_407,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case IOErrorEvent.IO_ERROR:
               this.debug("Load event IO ERROR for file \"" + this.url + "\"");
               if(!this.handleHttpStatus(this.var_407))
               {
                  this.var_1320 = getTimer();
         ...
         if(this.var_475 == LibraryLoader.const_1720)
         {
         ...
         if(this.var_475 == LibraryLoader.name_4)
         {
            if(this.prepareLibrary())
            {
               ...
               this.removeEventListeners();
               throttle();
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,this.var_407,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
      ...
      protected function prepareLibrary() : Boolean
      {
         ...
         this.var_648 = this.getDefinition(this._name) as Class;
         ...
         try
         {
            xmlClass = this.var_648.manifest as Class;
            if(xmlClass == null)
            {
               return false;
            }
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(this._debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,this.var_407,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         }
      }
      ...
      protected function failure(param1:String) : void
      {
         ...
         throttle();
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,this.var_407,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         dispatchEvent(new Event(LIBRARY_LOADER_FINALIZE));
      }
