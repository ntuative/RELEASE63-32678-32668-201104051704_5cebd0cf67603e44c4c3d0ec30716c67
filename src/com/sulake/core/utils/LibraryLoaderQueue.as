package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1595:int = 4;
      ...
      {
         return this.var_453.length + this.var_286.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_286.length < const_1595 && this.var_453.length > 0)
            {
               _loc1_ = this.var_453.shift();
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_453.indexOf(loader);
            if(index > -1)
            {
               this.var_453.splice(index,1);
            }
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_453)
            {
               if(param2)
               {
