package com.sulake.habbo.communication.demo
{
   ...
   public class HabboCommunicationDemo extends Component
   {
      ...
      private var var_2515:PseudoRandom;
      ...
      private const const_1743:String = "fuselogin";
      ...
      public function setSSOTicket(param1:String) : void
      {
         ...
         {
            this.var_631 = param1;
            this.var_58.initConnection(HabboConnectionType.const_223);
         }
      }
      ...
      private function onRoomSessionManagerReady(param1:IID, param2:IUnknown) : void
      {
         ...
         this.var_15.events.addEventListener(RoomSessionEvent.const_89,this.onRoomSessionEnded);
         this.checkRequirements();
      }
      ...
      {
         if(this.var_58 && this.var_165 && this.var_15 && this._windowManager)
         {
      ...
      private function componentsAreRunning() : void
      {
         ...
         var _loc1_:String = this.var_165.getKey("client.hotel_view.image");
         var _loc2_:String = this.var_165.getKey("image.library.url");
         if(_loc1_ != null && _loc2_ != null)
         {
            this.var_227.loadHotelViewImage(_loc2_ + _loc1_);
         }
         var _loc3_:* = this.var_165.getKey("use.sso.ticket") == "1";
         this.var_631 = this.var_165.getKey("sso.ticket",null);
         this.var_1828 = this.var_165.getKey("flash.client.url","");
         this.var_2514 = this.var_165.getKey("external.variables.txt","");
         if(this.var_227 == null)
         {
            this.var_227 = new HabboHotelView(this._windowManager,assets,Component(context).events);
         }
         if(_loc3_)
         {
            this.var_58.mode = HabboConnectionType.const_94;
            if(this.var_631)
            {
               this.var_58.initConnection(HabboConnectionType.const_223);
            }
         ...
         else if(this._windowManager != null)
         {
            ...
            if(_loc4_)
            {
               ...
               this._view.addEventListener(HabboLoginDemoView.const_509,this.onInitConnection);
            }
         }
      }
      ...
      private function onInitConnection(param1:Event = null) : void
      {
         ...
         {
            this.var_58.mode = HabboConnectionType.const_94;
            if(this.var_165.keyExists("local.environment"))
            {
               this.var_58.mode = HabboConnectionType.const_1052;
            }
         }
         this.var_58.initConnection(HabboConnectionType.const_223);
      }
      ...
      private function onInitCrypto(param1:IMessageEvent) : void
      {
         ...
         this.var_2515 = new PseudoRandom(parseInt(_loc4_.substring(_loc4_.length - 4),16),65536);
         switch(this.var_58.mode)
         {
            case HabboConnectionType.const_94:
               _loc6_ = "";
               _loc6_ = this.var_165.getKey("url.prefix",_loc6_);
               _loc6_ = _loc6_.replace("http://","");
               _loc6_ = _loc6_.replace("https://","");
               _loc7_ = this.var_165.getKey("hotelview.banner.url","http:/\nsitename$/gamedata/banner");
               _loc7_ = _loc7_.replace("$sitename$",_loc6_);
               this.var_1456 = _loc4_;
               this.var_227.loadBannerImage(_loc7_ + "?token=" + this.var_1456,this.onHotelViewBannerLoaded);
               break;
            case HabboConnectionType.const_1052:
               this.sendConnectionParameters(_loc2_);
               break;
            case HabboConnectionType.const_1692:
               this.var_952 = this.generateRandomHexString(30);
               this.var_521 = new DiffieHellman(new BigInteger(this.var_165.getKey("connection.development.prime"),16),new BigInteger(this.var_165.getKey("connection.development.generator"),16));
               this.var_521.init(this.var_952);
      ...
      private function onSecretKeyEvent(param1:IMessageEvent) : void
      {
         ...
         var _loc7_:IEncryption = new RC4_R27(_loc6_,this.var_2515);
      ...
      private function sendConnectionParameters(param1:IConnection) : void
      {
         ...
         this.var_1457 = true;
         this.dispatchLoginStepEvent(HabboCommunicationEvent.const_261);
         connection.send(new VersionCheckMessageComposer(401,this.var_1828,this.var_2514));
         var machineId:String = "";
         try
         {
            so = SharedObject.getLocal(this.const_1743,"/");
      ...
      private function onConnectionDisconnected(param1:Event) : void
      {
         ...
         if(this.var_1457)
         {
            this.dispatchLoginStepEvent(HabboCommunicationEvent.const_179);
         }
         if(param1.type == Event.CLOSE && !this.var_2516)
         {
            _loc2_ = this.var_165.getKey("logout.disconnect.url");
      ...
      private function onDisconnectReason(param1:DisconnectReasonEvent) : void
      {
         ...
         this.var_2516 = true;
         var _loc2_:String = this.var_165.getKey("logout.url");
         if(_loc2_.length > 0)
         {
      ...
      private function setOriginProperty(param1:String) : String
      {
         ...
         {
            return param1.replace("%origin%",this.var_165.getKey("flash.client.origin","popup"));
         }
         return param1;
      }
      ...
      private function onHotelViewBannerLoaded(param1:BitmapData) : void
      {
         ...
         var _loc4_:String = xor(_loc3_,this.var_1456);
