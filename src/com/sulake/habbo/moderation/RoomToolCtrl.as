package com.sulake.habbo.moderation
{
   ...
   public class RoomToolCtrl implements IDisposable, TrackedWindow
   {
      ...
      private var var_964:ICheckBoxWindow;
      
      private var var_1467:ICheckBoxWindow;
      ...
      public function show() : void
      {
         ...
         this.var_34.connection.send(new GetModeratorRoomInfoMessageComposer(this.var_381));
         Logger.log("BEGINNING TO SHOW: " + this.var_381);
      }
      
      public function getType() : int
      {
         return WindowTracker.const_1612;
      }
      ...
      {
         Logger.log("GOT ROOM INFO: " + param1.flatId + ", " + this.var_381);
         if(param1.flatId != this.var_381)
         {
            Logger.log("NOT THE SAME FLAT: " + param1.flatId + ", " + this.var_381);
            return;
         }
         this._data = param1;
         this.populate();
      ...
      public function populate() : void
      {
         ...
         this.var_1104 = IDropMenuWindow(this._frame.findChildByName("msgTemplatesSelect"));
         this.prepareMsgSelect(this.var_1104);
         this.var_1104.procedure = this.onSelectTemplate;
         this.var_964 = ICheckBoxWindow(this._frame.findChildByName("kick_check"));
         this.var_1467 = ICheckBoxWindow(this._frame.findChildByName("lock_check"));
         this.var_1466 = ICheckBoxWindow(this._frame.findChildByName("changename_check"));
      ...
      private function onChatlog(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_34.windowTracker.show(new ChatlogCtrl(new GetRoomChatlogMessageComposer(0,this._data.flatId),this.var_34,WindowTracker.const_841,this._data.flatId),this._frame,false,false,true);
      }
      ...
      {
         if(this.var_656 || this.var_542.text == "")
         {
            this.var_34.windowManager.alert("Alert","You must input a message to the user",0,this.onAlertClose);
            return;
         }
         var _loc2_:int = this.determineAction(param1,this.var_964.isSelected);
         this.var_34.connection.send(new ModeratorActionMessageComposer(ModeratorActionMessageComposer.const_1704,_loc2_,this.var_542.text,"",""));
         if(this.var_1467.isSelected || this.var_1466.isSelected || this.var_964.isSelected)
         {
            this.var_34.connection.send(new ModerateRoomMessageComposer(this._data.flatId,this.var_1467.isSelected,this.var_1466.isSelected,this.var_964.isSelected));
         }
         this.dispose();
      }
      ...
      private function determineAction(param1:Boolean, param2:Boolean) : int
      {
         ...
         return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1541);
      }
      ...
      private function onSelectTemplate(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         var _loc3_:String = this.var_34.initMsg.roomMessageTemplates[this.var_1104.selection];
         if(_loc3_ != null)
         {
            this.var_656 = false;
