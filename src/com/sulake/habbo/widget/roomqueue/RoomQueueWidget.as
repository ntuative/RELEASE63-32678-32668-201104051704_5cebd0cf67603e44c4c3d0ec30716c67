package com.sulake.habbo.widget.roomqueue
{
   ...
   public class RoomQueueWidget extends RoomWidgetBase
   {
      ...
      override public function registerUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.addEventListener(RoomWidgetRoomQueueUpdateEvent.const_569,this.onQueueStatus);
      ...
      override public function unregisterUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.removeEventListener(RoomWidgetRoomQueueUpdateEvent.const_569,this.onQueueStatus);
      }
      ...
      private function onQueueStatus(param1:RoomWidgetRoomQueueUpdateEvent) : void
      {
         ...
         if(param1.isActive)
         {
            this.var_1923 = param1.type;
            this.var_1499 = false;
            this.var_1217 = param1.position;
         }
         else
         {
            this.var_1499 = true;
         }
         this.var_2636 = param1.hasHabboClub;
         this.var_1924 = param1.isClubQueue;
         localizations.registerParameter("room.queue.position","position",this.var_1217.toString());
         localizations.registerParameter("room.queue.position.hc","position",this.var_1217.toString());
         localizations.registerParameter("room.queue.spectator.position","position",this.var_1217.toString());
         localizations.registerParameter("room.queue.spectator.position.hc","position",this.var_1217.toString());
         this.showInterface();
      }
      ...
      private function showInterface() : void
      {
         ...
         {
            switch(this.var_1923)
            {
               case RoomWidgetRoomQueueUpdateEvent.const_402:
                  _loc1_.caption = !!this.var_1924 ? "${room.queue.position.hc}" : "${room.queue.position}";
                  _loc2_.caption = "${room.queue.spectatormode}";
                  _loc3_.visible = this.var_1499;
                  break;
               case RoomWidgetRoomQueueUpdateEvent.const_569:
                  _loc1_.caption = !!this.var_1924 ? "${room.queue.spectator.position.hc}" : "${room.queue.spectator.position}";
                  _loc2_.caption = "${room.queue.back}";
                  _loc3_.visible = false;
            }
            _loc2_.visible = this.var_1499;
         }
         var _loc4_:IWindowContainer = this._window.findChildByName("club_container") as IWindowContainer;
         if(_loc4_ != null)
         {
            _loc4_.visible = !this.var_2636;
         }
      ...
      private function exitQueue(param1:WindowMouseEvent) : void
      {
         ...
         var _loc2_:RoomWidgetRoomQueueMessage = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_665);
      ...
      {
         messageListener.processWidgetMessage(new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_745));
      }
      ...
      private function changeQueue(param1:WindowMouseEvent) : void
      {
         ...
         if(this.var_1923 == RoomWidgetRoomQueueUpdateEvent.const_402)
         {
