package com.sulake.habbo.widget.poll
{
   ...
   public class PollWidget extends RoomWidgetBase
   {
      ...
      override public function dispose() : void
      {
         ...
         if(this.var_315)
         {
            _loc1_ = this.var_315.length;
            _loc2_ = 0;
            while(_loc2_ < _loc1_)
            {
               _loc3_ = this.var_315.getWithIndex(0) as PollSession;
               if(_loc3_ != null)
               {
                  _loc3_.dispose();
               }
      ...
      override public function registerUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.addEventListener(RoomWidgetPollUpdateEvent.const_141,this.showPollOffer);
         param1.addEventListener(RoomWidgetPollUpdateEvent.const_73,this.showPollError);
         param1.addEventListener(RoomWidgetPollUpdateEvent.const_148,this.showPollContent);
      ...
      override public function unregisterUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.removeEventListener(RoomWidgetPollUpdateEvent.const_141,this.showPollOffer);
         param1.removeEventListener(RoomWidgetPollUpdateEvent.const_73,this.showPollError);
         param1.removeEventListener(RoomWidgetPollUpdateEvent.const_148,this.showPollContent);
      }
      
      private function showPollOffer(param1:Event) : void
      {
         var _loc2_:int = RoomWidgetPollUpdateEvent(param1).id;
         var _loc3_:PollSession = this.var_315.getValue(_loc2_) as PollSession;
      ...
      private function showPollContent(param1:Event) : void
      {
         ...
         {
            _loc3_ = _loc2_.id;
            _loc4_ = this.var_315.getValue(_loc3_) as PollSession;
            if(_loc4_)
            {
      ...
      {
         var _loc2_:PollSession = this.var_315.getValue(param1) as PollSession;
         if(_loc2_)
         {
            _loc2_.showThanks();
            _loc2_.dispose();
            this.var_315.remove(param1);
         }
      }
