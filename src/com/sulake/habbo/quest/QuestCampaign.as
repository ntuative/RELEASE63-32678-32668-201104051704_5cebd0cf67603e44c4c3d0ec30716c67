package com.sulake.habbo.quest
{
   ...
   public class QuestCampaign implements IDisposable
   {
      ...
      private var var_526:Quest;
      
      private var var_206:QuestView;
      ...
      private var var_313:IItemListWindow;
      ...
      public function dispose() : void
      {
         ...
         for each(_loc1_ in this.var_782)
         {
            _loc1_.dispose();
         }
         this.var_782 = [];
      ...
      private function reset() : void
      {
         ...
         if(this.var_312)
         {
            ...
            {
               this.var_313.removeListItem(_loc1_.view);
               _loc1_.dispose();
            }
            this.var_312 = null;
            this.var_313.destroyListItems();
            this.var_313.removeListItems();
         }
         this.var_526 = null;
      }
      ...
      private function prepareWindow() : void
      {
         ...
         this.var_1470 = this._window.findChildByName("currency.container") as IWindowContainer;
         this.var_313 = this._window.findChildByName("quests") as IItemListWindow;
         var _loc1_:* = "icon_" + this.code + "_pixeltype";
         var _loc2_:IBitmapWrapperWindow = this.var_1470.findChildByName("currency.icon") as IBitmapWrapperWindow;
         this._controller.questEngine.setImageFromAsset(_loc2_,_loc1_,this.onPreviewImageReady);
         this.var_1470.findChildByName("currency.amount").caption = "${quest_" + this.var_1858 + ".currency}";
         var _loc3_:String = "quest_" + this.code;
         var _loc4_:* = _loc3_ + ".title";
         var _loc5_:* = _loc3_ + ".currency";
         this.setText("currency.amount",_loc5_);
         this.var_783 = this._window.findChildByName("help.button") as IWindowContainer;
         this.var_1185 = this._window.findChildByName("help.container") as IRegionWindow;
         if(this.var_1185)
         {
            this.var_1185.addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK,this.helpButtonEventProc);
            this.var_1185.addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_OVER,this.helpButtonEventProc);
            this.var_1185.addEventListener(WindowMouseEvent.const_25,this.helpButtonEventProc);
         }
         if(this.var_783)
         {
            this.var_783.color = 14737632;
         }
      }
      
      public function attachTo(param1:IWindowContainer) : void
      {
         if(this.var_825 || !this._window)
         {
            return;
         }
         ...
         if(this.var_167)
         {
            ...
            if(this.var_206)
            {
               this.var_206.view.height = this.var_167.height;
            }
         }
      }
      ...
      private function openHelp() : void
      {
         ...
         if(this.var_526)
         {
            this.var_526.selected = false;
         }
         if(this.var_129)
         {
            this.var_167.addChild(this.var_129.view);
         }
         if(this.var_167)
         {
            this.var_167.height = this._window.height - this.var_167.y * 2;
            ...
            {
               this.var_129.view.height = this.var_167.height;
            }
         }
      }
      ...
      private function onPreviewImageReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(_loc2_ != null)
         {
            _loc3_ = this.var_1470.findChildByName("currency.icon") as IBitmapWrapperWindow;
      ...
      public function openCampaign(param1:Array) : void
      {
         ...
         {
            this.var_312.push(new Quest(_loc2_,this));
         }
         _loc3_ = null;
         for each(_loc4_ in this.var_312)
         {
            this.var_313.addListItem(_loc4_.view);
            if(!_loc3_ && (_loc4_.status == QuestStatusEnum.const_46 || _loc4_.status == QuestStatusEnum.const_373))
            {
               _loc3_ = _loc4_;
            }
         }
         if(_loc3_)
         {
            ...
            if(this.var_313.maxScrollV > 0)
            {
               this.var_313.scrollV = _loc3_.view.y / this.var_313.maxScrollV;
            }
         }
         else if(this.var_312.length > 0)
         {
            this.activateListItem(this.var_312[0]);
         }
      }
      ...
      public function activateListItem(param1:Quest) : void
      {
         ...
         this.var_526 = param1;
         this.var_526.selected = true;
         this.showQuestData(this.var_526);
      }
      ...
      private function showQuestData(param1:Quest) : void
      {
         ...
         this.var_206 = new QuestView(this,param1);
         this.var_167.addChild(this.var_206.view);
         this.var_206.init();
         this.var_167.height = this._window.height - this.var_167.y * 2;
         this.var_206.view.height = this.var_167.height;
      }
      ...
      {
         this.modifyQuestData(param1);
         this.var_782.push(new QuestCompleted(this,param1));
      }
      ...
      public function closeQuestCompleted(param1:QuestCompleted) : void
      {
         ...
         var _loc3_:* = null;
         var _loc4_:int = 0;
         for each(_loc3_ in this.var_782)
         {
            if(_loc3_ == param1)
            {
               _loc2_ = _loc3_;
         ...
         {
            _loc2_.dispose();
            _loc4_ = this.var_782.indexOf(_loc2_);
            if(_loc4_ >= 0)
            {
               this.var_782.splice(_loc4_,1);
            }
         }
      }
      ...
      private function modifyQuestData(param1:QuestMessageData) : void
      {
         ...
         if(_loc2_)
         {
            ...
            this.var_312[_loc3_] = _loc5_;
            _loc6_ = this.var_313.getListItemIndex(_loc2_.view);
            this.var_313.removeListItem(_loc2_.view);
            if(_loc6_ > -1)
            {
               this.var_313.addListItemAt(_loc5_.view,_loc6_);
            }
            else
            {
               this.var_313.addListItem(_loc5_.view);
            }
            if(this.var_526 == _loc2_)
            {
