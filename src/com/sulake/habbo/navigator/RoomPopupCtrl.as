package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2401 = new RoomThumbnailRenderer(param1);
         this.var_1191 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_185 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_185.visible = true;
         Util.hideChildren(this.var_185);
         this.refreshRoomName(this.var_185,this.var_113);
         if(this.var_113.ownerName != "" && this.var_113.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_185,!!this.var_113.event ? this.getEventCtg(this.var_113.categoryId) : this.getRoomCtg(this.var_113.categoryId));
         this.refreshRoomDesc(this.var_185,this.var_113);
         this.refreshExtraCont();
         this.refreshInfo(this.var_185,"trading_allowed",this.var_113.allowTrading);
         this.refreshInfo(this.var_185,"doormode_doorbell",this.var_113.doorMode == 1);
         this.refreshInfo(this.var_185,"doormode_password",this.var_113.doorMode == 2);
         Util.moveChildrenToColumn(this.var_185,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_185.height = Util.getLowestPoint(this.var_185);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_185,this.var_113.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_185.findChildByName("roomowner"));
         if(this.var_113.ownerName != "" && this.var_113.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_185.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1191.refreshTags(_loc1_,this.var_113.tags);
         ...
         this.var_2401.refreshThumbnail(_loc1_,this.var_113.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_248,Tabs.const_258,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
