package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_845:ColorTransform;
      
      private var var_1566:ColorTransform;
      
      private var var_2033:Matrix;
      
      private var _translationMatrix:Matrix;
      
      private var var_786:Number = 1.0;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_845 = new ColorTransform();
         this.var_845.alphaMultiplier = 1;
         this.var_1566 = new ColorTransform();
         this.var_2033 = new Matrix();
      ...
      {
         var _loc1_:* = null;
         for each(_loc1_ in this.var_1283)
         {
            _loc1_.dispose();
         }
         this.var_1283 = null;
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_100 = this.var_1283[param1];
         this.var_1046 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_2034 >= 0)
         {
            this.var_284 = this._visualization.getSprite(this.var_2034);
            if(this.var_284 && this.var_284.asset)
            {
               ...
               if(this.var_88 == null)
               {
                  this.var_88 = this.var_284.asset.clone();
                  if(this.var_845.alphaMultiplier != 1)
                  {
                     this.var_689 = new BitmapData(this.var_88.width,this.var_88.height,true,4278190080);
                  }
               }
               this.var_2410 = -this.var_284.offsetX;
               this.var_2409 = -this.var_284.offsetY;
               this.var_284.asset = this.var_88;
            }
            if(this.var_88)
            {
            ...
            if(this.var_689)
            {
               this.var_689.fillRect(this.var_689.rect,4278190080);
            }
         }
      }
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_100.y * this.var_1284;
         }
         return 0;
      }
      ...
      {
         if(!this.var_100 || !this.var_284)
         {
            return;
         }
         if(this.var_88 && this.var_284.asset != this.var_88)
         {
            this.var_284.asset = this.var_88;
         }
         if(this.var_1046)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_1046 && this.var_100.hasIgnited)
         {
            this.var_1046 = true;
         }
         _loc8_ = this.var_1205 * this.var_1284;
         this.var_100.update();
         if(this.var_1046)
         {
            if(this.var_100.roomObjectSpriteId >= 0)
            ...
            {
               this.var_88.draw(this.var_689,this.var_2033,this.var_845,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_100.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2410 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1284;
               _loc4_ = this.var_2409 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1284;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_88.draw(_loc7_,this._translationMatrix,this.var_1566,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2821 = parseInt(param1.@size);
         this.var_2034 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1205 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1284 = this.var_2821 / 64;
         this.var_786 = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this.var_786 = Math.min(this.var_786,1);
         this.var_845.alphaMultiplier = this.var_786;
